...
local image_bg = cairo_image_surface_create_from_png("clock/BG3.png")
...
function conky_clock()
    ...
    cairo_save(cr)
    cairo_translate(cr, -21, -20)
    cairo_scale(cr, 252 / w, 175 / h) -- Changed to match original scaling
    cairo_set_source_surface(cr, image_bg, 0, 1)
    cairo_paint(cr)
    cairo_restore(cr)

    -- Digit positions and layout
    local x_positions = {25, 65, 120, 160} -- HH MM
    local y_base = 28
